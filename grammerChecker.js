...
Getting Started
Q1. In main.js, the story variable holds the paragraph we’ll be editing. In order to edit the story, we turn it into an array on line 3. The .split() method separates the story string by the space character (' ') and stores each word as an element of the array.
...
Q2. For a better visual comparison of the original and edited stories, we want to view the edited storyWords array as a string. To change the storyWords array back into a readable string, we can invoke the .join() method on storyWords.
...
Counting Words
Q3.Now it’s time to start editing the story by manipulating the storyWords array. We want to be able to see the changes, so be sure your console.log() of the joined story is the last line of code in your editor.
...
Q4.For each word in the storyWords array, we want the count variable to increment by one.
...
Q6.Below the storyWords variable declaration, notice the variable unnecessaryWord on line 4. We want to filter out the value of unnecessaryWord from the story.
...
Replacing Words
Q7.Now that we’ve removed the unnecessary words, let’s take care of any misspelled words in the story. We can reassign storyWords to a new array of spell-checked words using the .map() method!
...
Q8. Uh oh, your great grandmother is going to read the story and there’s a “bad” word in it! Let’s apply the .findIndex() method to storyWords to find the index of the bad word.
...
Q9.Now that we have the index of the bad word, we can easily replace it with something more appropriate.
...
Q10. Finally, let’s simplify the words in the story to appeal to a broader audience. We can make sure every word in the story is 10 characters or less using the .every() method. The .every() method uses a callback function to test if every element in an array passes a specified condition. It returns true if all elements pass, and false if there is an element that does not pass.
...
Q11. Hmm, it looks like there’s at least one word longer than 10 characters.
...
let storyWords = story.split(' '); // uses .split method to create an empty array, each element in array will be
...
let count = 0; //create count variable = 0, and we're going, goal here is to increment this count vaiable by 1.
storyWords.forEach((word) => {// start by creating a forEach 'method' on the storyWords array. Using ES6 syntax for arrow function here.
    count++; // increment count by 1.
});
console.log(count); // output 181

storyWords = storyWords.filter((word) => {//here, we re-assign storyWords variable, callback function with parameter 'word'
    return word !== unnecessaryWord; //!== means Does NOT = unnessaryWord

});

storyWords = storyWords.map((word) => {
    return word === misspelledWord ? 'beautiful' // return correct spelling
    : word; // : means else
})

let badWordIndex = storyWords.findIndex((word) => { //callback function with 'word' parameter
    return word === badWord;
})
console.log(badWordIndex); //78 index is where the bad word is located.

storyWords[78] = 'really';
let lengthCheck = storyWords.every((word) => { // create variable called lengthCheck , then create callback function with parameter 'word'
    return word.length < 10;
});
console.log(lengthCheck); // output false, which means there is a word less than 10 characters long.
